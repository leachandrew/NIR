...
df1 <- plant_data %>% 
  filter(lfe) %>% 
  group_by(ref_year)%>%
  mutate(natl_lfe=sum(total_co2e)/10^6)%>%
  group_by(prov,ref_year,naics2_desc,natl_lfe) %>% 
  summarise(sector_lfe=sum(total_co2e,na.rm = T)/10^6,prov_ghgs=mean(GHGs),prov_pop=max(Prov_pop))%>% ungroup()%>%
  group_by(naics2_desc,ref_year) %>% 
  #mutate(natl_sector_lfe=sum(sector_lfe,na.rm=T)) %>% 
  ungroup()%>%
  mutate(sector=as_factor(naics2_desc), sector=fct_reorder(sector,-sector_lfe))%>%
  mutate(sector=fct_other(sector,keep = levels(sector)[1:5],  other_level = "Other")) %>%
  ungroup()%>% 
  group_by(prov,sector,ref_year) %>% 
  summarize(natl_lfe=unique(natl_lfe),
            prov_sector_lfe=sum(sector_lfe),
            #natl_sector_lfe=sum(natl_sector_lfe),
            prov_ghgs=unique(prov_ghgs),
            prov_pop=unique(prov_pop)/10^6) %>% ungroup() %>% 
  group_by(prov,ref_year) %>% 
  mutate(prov_lfe_ghg=sum(prov_sector_lfe),prov_other_ghg=prov_ghgs-prov_lfe_ghg) %>% ungroup()


ggplot(df1)+
  geom_line(aes(ref_year,prov_ghgs),colour="red",size=3)+
  geom_line(aes(ref_year,prov_lfe_ghg),colour="blue")+
  geom_line(aes(ref_year,prov_other_ghg),colour="green")+
  geom_line(aes(ref_year,(prov_lfe_ghg+prov_other_ghg)),colour="orange")+
  facet_grid( ~ prov)



df2<-df1 %>% ungroup()%>% pivot_wider(names_from=sector,values_from=prov_sector_lfe)%>%
  pivot_longer(cols=-c("prov","ref_year","natl_lfe","prov_ghgs","prov_lfe_ghg","prov_pop"),values_to = "prov_sector_ghgs",names_to = "sector")%>%
  mutate(sector=fct_recode(sector,"Small Emitters"="prov_other_ghg"),
         sector=fct_recode(sector,"Other Large Emitters"="Other"),
         sector=fct_relevel(sector,"Other Large Emitters",after = Inf),
...
#natl<-df1 %>% group_by(ref_year,prov) %>% summarize(lfe_ghg=mean(lfe_ghg),prov_ghgs=mean(prov_ghgs)) %>% ungroup()%>%
...
#library(ggrepel)
ggplot(df1)+
  geom_area(aes(ref_year,prov_sector_ghgs,fill=sector),colour="black",position = "stack",size=.1)+
  ...
  geom_label(data=df1 %>% filter(ref_year==2021,sector=="Transportation"),
                   aes(x=2004+(2021-2004)/2,
                       y=320,label=str_wrap(label, width = 20)),
                   fontface = 'bold',
                   size = 2.8,
...
ggplot(#test<-
  df1%>%filter(sector=="Small Emitters",prov!="TERR")%>%
         group_by(prov)%>%
    mutate(roc=(prov_sector_ghgs/first(prov_sector_ghgs)),
           lfe=(prov_lfe_ghg/first(prov_lfe_ghg)))%>%
    filter(ref_year>2004)
       )+
  geom_line(aes(ref_year,(roc-1)*100,color="Small Emitters"),size=1.1)+
  geom_line(aes(ref_year,(lfe-1)*100,color="Large Emitters"),size=1.1)+
  #geom_line(aes(Ref_Year,`Provincial Emissions`),size=2)+
  facet_grid( ~ prov)+
  scale_x_continuous(breaks=pretty_breaks())+
  scale_y_continuous(breaks=pretty_breaks())+
  scale_color_manual("",values = colors_ua10()[-2],guide = "legend")+
  theme_minimal()+
  theme(
    plot.margin = margin(.5, .5, .5, .5, "cm"),
  ...
  labs(x=NULL,y=expression('Change in Emissions Since 2004 '*'(%)'),
